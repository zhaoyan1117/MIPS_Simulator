...
uint32_t signext(int unext){
    int signext;
    if (unext >> 15 == 0)
    	signext = unext;
    else
    	signext = unext | 0xffff0000;
    return signext;
}
...
void execute_one_inst(processor_t* p, int prompt, int print_regs)
{
  ...
  case 0x0: // opcode == 0x0 (SPECIAL)
    ...
    case 0x2: // funct == 0x2 (SRL)
      p->R[inst.rtype.rd] = (reg_t) ((unsigned int) p->R[inst.rtype.rt] >> inst.rtype.shamt);   //0 extension.
      p->pc += 4;
      break;

    case 0x3: // funct == 0x3 (SRA)
      p->R[inst.rtype.rd] = (reg_t) ((signed int) p->R[inst.rtype.rt] >> inst.rtype.shamt);   //sign extension.
      p->pc += 4;
      break;
  ...
  case 0x9: // opcode == 0x9 (ADDIU)
    p->R[inst.rtype.rt] = (unsigned int) p->R[inst.rtype.rs] + (unsigned int) signext(inst.itype.imm);
    p->pc += 4;
    break;

  case 0xa: // opcode == 0xa (SLTI)
	p->R[inst.rtype.rt] = (signed int) p->R[inst.rtype.rs] < (signed int) signext(inst.itype.imm);
    p->pc += 4;
    break;
  ...
  case 0x20: // opcode == 0x20 (LB)
    p->R[inst.itype.rt] = signext( load_mem( p->R[inst.itype.rs] + signext(inst.itype.imm), SIZE_BYTE ) );
    p->pc += 4;
    break;

  case 0x21: // opcode == 0x21 (LH)
    p->R[inst.itype.rt] = signext( load_mem( p->R[inst.itype.rs] + signext(inst.itype.imm), SIZE_HALF_WORD ) );
    p->pc += 4;
    break;
  ...
  case 0x25: // opcode == 0x25 (LHU)
    p->R[inst.itype.rt] = load_mem( p->R[inst.itype.rs] + signext(inst.itype.imm), SIZE_HALF_WORD );
    p->pc += 4;
    break;
  ...
  case 0x29: // opcode == 0x29 (SH)
    store_mem( p->R[inst.itype.rs] + signext(inst.itype.imm), SIZE_HALF_WORD, p->R[inst.itype.rt] );
    p->pc += 4;
    break;
